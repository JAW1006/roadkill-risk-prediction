...
| GNN 착수 전 학습 주간 | ⬜ 시작 전 | 1주차(9/10~9/16), PyG 공식 튜토리얼(Cora) 팀 전체 실습 |
| GNN (GCN·GraphSAGE) | ⬜ 시작 전 | 베이스라인 실데이터 학습 이후. **체크포인트: 8주차 금요일(10/30)까지 베이스라인 PR-AUC 미달 시 발표는 베이스라인으로 진행** |
| RAG 파이프라인 | ⬜ 시작 전 | |
...
**GNN 착수 전 리스크 관리** (근거: `DESIGN_DECISIONS.md` §5):
- 1주차(9/10~9/16)에 PyTorch Geometric 공식 튜토리얼(Cora 노드분류)을 팀
  전체가 손으로 따라 쳐보는 학습 기간을 먼저 갖는다 — 목표는 "에러 메시지를
  봤을 때 감이라도 잡히는 정도"
- 팀을 2트랙으로 분리: **A(데이터·베이스라인·대시보드)** / **B(그래프·GNN)**
  — 트랙별 인원 배정은 팀 내부에서 확정 필요
- **체크포인트: 8주차 금요일(2026-10-30)**까지 GNN이 베이스라인 PR-AUC를
  넘지 못하면 발표는 베이스라인 결과로 진행하고, GNN은 "시도했으나 이런
  이유로 한계가 있었다"로 브리핑에 포함
- 그래프 범위는 처음부터 특정 도(道) 하나 또는 고속도로 1~2개 노선으로 좁게
  시작, 검증되면 확장 (전국 단위로 바로 시작하지 않음)
...
0. **(병행, 1주차 중)** GNN 학습 주간 — PyG 공식 튜토리얼(Cora) 팀 전체 실습, 2트랙(A: 데이터·베이스라인·대시보드 / B: 그래프·GNN) 인원 배정 확정 (근거: `DESIGN_DECISIONS.md` §5)
...
5. **GNN 착수** — 그래프 범위는 도(道) 하나 또는 고속도로 1~2개 노선으로 좁게 시작. **체크포인트: 8주차 금요일(2026-10-30)**까지 베이스라인 대비 개선 없으면 발표는 베이스라인으로 확정
...
| 2026-09-10 | `233e390` | Streamlit 대시보드 목업 뼈대 |
